...
function flattenTokens(
  obj: TokenData,
  prefix: string = '',
  result: Map<string, TokenValue> = new Map()
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value && typeof value === 'object' && 'value' in value && 'type' in value) {
      ...
      result.set(path, value as TokenValue);
    } else if (value && typeof value === 'object') {
      // This is a nested object, recurse
      flattenTokens(value as TokenData, path, result);
    }
  }
  return result;
}
...
function extractColorValue(value: string): RGB | RGBA | null {
...
function buildPrimitiveMap(obj: TokenData, prefix: string = '', result: Map<string, TokenValue> = new Map()): Map<string, TokenValue> {
  for (const [key, value] of Object.entries(obj)) {
    const path = prefix ? `${prefix}.${key}` : key;
    if (value && typeof value === 'object' && 'value' in value && 'type' in value) {
      result.set(path, value as TokenValue);
    } else if (value && typeof value === 'object') {
      buildPrimitiveMap(value as TokenData, path, result);
    }
  }
  return result;
}
...
function findPrimitiveVariable(refPath: string): Variable | null {
...
function parseDecimalNotation(value: string): number {
...
function getPrimitiveVariableAlias(refPath: string): VariableAlias | null {
...
function getPrimitiveNumberValue(refPath: string): number | null {
  const variable = findPrimitiveVariable(refPath);
  if (variable && variable.resolvedType === 'FLOAT') {
    ...
    if (collection && collection.modes.length > 0) {
      ...
      if (defaultValue && typeof defaultValue === 'object' && 'type' in defaultValue && defaultValue.type === 'VARIABLE_ALIAS') {
        const refVar = figma.variables.getVariableById((defaultValue as VariableAlias).id);
...
function resolveColorValueRecursive(colorValue: VariableAlias | RGB | RGBA, depth: number = 0): RGB | RGBA | null {
  ...
  if ('r' in colorValue && 'g' in colorValue && 'b' in colorValue) {
    return colorValue as RGB | RGBA;
  }
  
  // If it's a VariableAlias, resolve it
  if ('type' in colorValue && colorValue.type === 'VARIABLE_ALIAS') {
    try {
      const refVar = figma.variables.getVariableById((colorValue as VariableAlias).id);
      if (refVar && refVar.resolvedType === 'COLOR') {
        const refCollection = figma.variables.getVariableCollectionById(refVar.variableCollectionId);
        if (refCollection && refCollection.modes.length > 0) {
          const refColorValue = refVar.valuesByMode[refCollection.modes[0].modeId];
          if (refColorValue) {
            return resolveColorValueRecursive(refColorValue as VariableAlias | RGB | RGBA, depth + 1);
          }
        }
      }
    } catch (error) {
...
function findSemanticVariable(refPath: string): Variable | null {
  // Sanitize the reference path to match Figma variable naming
  const sanitizedName = sanitizeVariableName(refPath);
...
function findSemanticPositionVariable(refPath: string): Variable | null {
  return findSemanticVariable(refPath);
}

// Get all font token variables from Semantic collection
function getSemanticFontTokens(): Array<{ variable: Variable; path: string; value: string }> {
  const semanticCollection = figma.variables.getLocalVariableCollections()
    .find(c => c.name === 'Semantic' || c.name === 'Semantics');
  ...
  const fontTokens: Array<{ variable: Variable; path: string; value: string }> = [];
  const allVariables = figma.variables.getLocalVariables();
...
async function processFontStylesFromSemantic(primitiveMap?: Map<string, TokenValue>): Promise<{
...
function resolvePrimitiveColorToRGB(refPath: string): RGB | RGBA | null {
  ...
  if (variable && variable.resolvedType === 'COLOR') {
    ...
    if (collection && collection.modes.length > 0) {
      ...
      if (colorValue) {
        const resolved = resolveColorValueRecursive(colorValue as VariableAlias | RGB | RGBA);
        if (resolved) {
          return resolved;
        }
      }
...
function ensureShadowVariable(
  refPath: string,
  resolvedValue: number | RGB | RGBA,
  variableType: 'FLOAT' | 'COLOR',
  primitiveMap?: Map<string, TokenValue>
): Variable | null {
  ...
  try {
    ...
    for (const mode of collection.modes) {
      if (variableType === 'FLOAT' && typeof resolvedValue === 'number') {
        ...
        newVar.setValueForMode(mode.modeId, resolvedValue as RGB | RGBA);
      }
    }
...
// Parse font token value string into font properties
...
function parseFontValue(value: string): FontProperties | null {
  if (!value || value === 'none') {
    return null;
  }
...
function getPrimitiveFontValue(refPath: string): number | null {
  const variable = findPrimitiveVariable(refPath);
  if (variable && variable.resolvedType === 'FLOAT') {
    const collection = figma.variables.getVariableCollectionById(variable.variableCollectionId);
    if (collection && collection.modes.length > 0) {
...
function getPrimitiveFontFamilyValue(refPath: string): string | null {
  const variable = findPrimitiveVariable(refPath);
...
function getPrimitiveFontVariableAlias(refPath: string, type: 'FLOAT' | 'STRING'): VariableAlias | null {
  const variable = findPrimitiveVariable(refPath);
...
async function createTextStyleFromFont(
  styleName: string,
  ...
  let resolvedFontSize: number | null = fontSizeValue;
  ...
  let fontFamily: string = 'Inter'; // default
  ...
  // Get font weight
  let fontWeight: number = 400; // default
  let fontWeightStyle: string = 'Regular'; // default style name
  if (fontProps.fontWeight) {
    ...
    if (fontWeightValue !== null) {
      ...
      if (fontWeight === 100) fontWeightStyle = 'Thin';
      else if (fontWeight === 200) fontWeightStyle = 'Extra Light';
      else if (fontWeight === 300) fontWeightStyle = 'Light';
      else if (fontWeight === 400) fontWeightStyle = 'Regular';
      else if (fontWeight === 500) fontWeightStyle = 'Medium';
      else if (fontWeight === 600) fontWeightStyle = 'Semi Bold';
      else if (fontWeight === 700) fontWeightStyle = 'Bold';
      else if (fontWeight === 800) fontWeightStyle = 'Extra Bold';
      else if (fontWeight === 900) fontWeightStyle = 'Black';
      else fontWeightStyle = fontWeight.toString();
    }
  }
  
  // Get line height - Always resolve to actual pixel value (never use VariableAlias)
  let lineHeight: { value: number; unit: 'PIXELS' } | null = null;
  ...
  let letterSpacing: { value: number; unit: 'PERCENT' } | null = null;
  ...
  let paragraphSpacing: number | null = null;
...
function createEffectStyleFromShadows(
  styleName: string,
  ...
  for (const shadow of shadows) {
    ...
    const x: number = xValue !== null ? xValue : parseDecimalNotation(shadow.x);
    
    const yValue = getPrimitiveNumberValue(`position.y.${shadow.y}`);
    const y: number = yValue !== null ? yValue : parseDecimalNotation(shadow.y);
    
    const blurValue = getPrimitiveNumberValue(`blur.${shadow.blur}`);
    const blur: number = blurValue !== null ? blurValue : parseDecimalNotation(shadow.blur);
    
    const spreadValue = getPrimitiveNumberValue(`spread.${shadow.spread}`);
    const spread: number = spreadValue !== null ? spreadValue : parseDecimalNotation(shadow.spread);
    
    // Resolve color to RGB/RGBA
    let color: RGB | RGBA = { r: 0, g: 0, b: 0, a: 1 }; // default to black
...
function resolveTokenValue(
  ...
  primitiveMap?: Map<string, TokenValue>,
  originalToken?: TokenValue
): VariableAlias | RGB | RGBA | null {
  if (!value) {
  ...
  if (refMatch) {
    ...
    let resolvedToken: TokenValue | undefined;
...
function testAttachVariablesToEffects() {
  ...
  for (const style of effectStyles) {
    if (testAttempts >= 3) break; // Test first 3 styles only
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'create-variables') {
    try {
      ...
      let primitiveMap: Map<string, TokenValue> | undefined;
      if (primitiveData) {
      ...
      for (const path of boxShadowPaths) {
        ...
        if (!lightToken || !darkToken) continue;
        
        // Skip "none" tokens
        if (lightToken.value === 'none' || darkToken.value === 'none') continue;
  ...
  if (msg.type === 'create-font-styles') {
    ...
    try {
      ...
      let semanticTokens: Map<string, TokenValue>;
      ...
      try {
        ...
        if (fontPaths.length > 0) {
          ...
          if (hasFontKey) {
            addLog(`Font object keys: ${Object.keys((semanticData as any).font || {}).slice(0, 10).join(', ')}`);
          }
